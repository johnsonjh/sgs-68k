...
/*	ATT: #ident	"sgs-inc:common/filehdr.h	1.19"	*/


#ifndef FILEHDR_H
#ident	"@(#)filehdr.h	24.1	10/28/91 Copyright (c) 1991 by Arix Corp."
#define FILEHDR_H
...
#define  F_POSIX	0100000
#define  F_S3000	040000
...
/* BEGIN ARETE DEFINES */
#define M68ASAMAGIC	051501	/* "SA" arete standalone linked object */
#define M68MIMAGIC	MC68MAGIC 	/* unlinked object */
#define M68AWRMAGIC	0570	/* arete normal file */
#define M68AROMAGIC	0575	/* arete pure file */
#define OM68NWRMAGIC	0610	/* ncr normal file old pcc style*/
#define OM68NROMAGIC	0615	/* ncr pure file old pcc style*/
#define M68NWRMAGIC	0620	/* ncr normal file pcc2 style */
#define M68NROMAGIC	0625	/* ncr pure file pcc2 style */
/* 68020 MAGIC NUMBERS */
#define A68020MAGIC	0522	/* local 68020 Magic number */
#define N68020MAGIC	0630
#define X68020MAGIC	0720	/* common 68020 Magic number */

/* 68040 MAGID NUMBERS */
#define M68040MAGIC_OS	0523	/* M68040 OS and iopm kernels */

/* and make the equivalents for the ncr names */
#define ONCRWRMAGIC OM68NWRMAGIC
#define ONCRROMAGIC OM68NROMAGIC
#define NCRWRMAGIC M68NWRMAGIC
#define NCRROMAGIC M68NROMAGIC
/* END ARETE DEFINES */
...
#define ISCOFF(x) \
		...
		|| ((x)==IAPX16) || ((x)==IAPX16TV) \
		|| ((x)==IAPX20) || ((x)==IAPX20TV) \
		|| ((x)==U370WRMAGIC) || ((x)==U370ROMAGIC) \
		|| ((x)==MTVMAGIC) \
		|| ((x)==I286SMAGIC) || ((x)==I286LMAGIC) \
		|| ((x)==MC68KWRMAGIC) || ((x)==MC68KROMAGIC) \
		|| ((x)==A68020MAGIC) || ((x)==N68020MAGIC) \
		|| ((x)==X68020MAGIC) || ((x)==M68ASAMAGIC) \
		|| ((x)==M68MIMAGIC) || ((x)==M68AWRMAGIC) \
		|| ((x)==M68AROMAGIC) || ((x)==OM68NWRMAGIC) \
		|| ((x)==OM68NROMAGIC) || ((x)==M68NWRMAGIC) \
		|| ((x)==M68NROMAGIC) || ((x)==MC68KPGMAGIC) \
		|| ((x)==M68040MAGIC_OS))
