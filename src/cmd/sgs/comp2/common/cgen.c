#ident	"@(#)cgen.c	25.1	12/2/91 Copyright (c) 1991 by Arix Corp."
/*	@(#)cgen.c	7.1		*/
...
static int  c2bigger _PARMS((union ndu *));
...

{
	...
	** GENBR and GENSCC have left as Condition codes
...

{
	...
	return ( volatile_chk(p) );			/* bk */
}

volatile_chk( p )
NODE *p;
{
	/* return 1 if p uses a volatile, 0 otherwise */
	int o;
	o = p->tn.op;
	...
	{
	case BITYPE:
		if( volatile_chk( p->in.right ) ) return( 1 );
	case UTYPE:
		return( volatile_chk( p->in.left ) );
	}

	if ( is_volatile( p->tn.typeq ) )
		return( 1 );
	return( 0 );
}
...

{
	...
	t = p->tn.type;
	if (   ( h->tn.type == TFLOAT || h->tn.type == TDOUBLE )
		&& ( t != TFLOAT && t != TDOUBLE )
	   )
		/* mixed types, convert to float */
		t = TDOUBLE;
	else if( t == TCHAR || t == TSHORT ) t = TINT;
...

{
	...
	if( !asgop(o) || o==ASSIGN ) 
	{
		if( o==ASSIGN ) 
		{
			...
			if( o==RNODE || o==QNODE || o==SNODE ) 
			{
				/* force into r0 */
				p->in.left->tn.op = REG;
				/* fix talloc compile problem with QNODEs d.t. */
				p->in.left->tn.rval = o==QNODE ? 1 : callreg( p->in.right );
				p->in.left->tn.lval = 0;	/* bk: broke M4.1 USE_AREG */
				if( odebug ) 
				{
		...
		else 
		{
			...
			/* check for inserted GENSCC long/unsigned mismatch */
			if (   (t==TINT && t2==TUNSIGNED)
				|| (t2==TINT && t==TUNSIGNED)
			   )
			  {	p->in.left->tn.type = TUNSIGNED;
				p->in.right->tn.type = TUNSIGNED;
				return ( 0 );
				}
			...
			p->in.left = ind2type( p, p->in.left );
			p->in.right = ind2type( p, p->in.right );
			if( odebug ) {
	...
	if( o == INCR || o == DECR ) 
	{
		...
		/* Motorola 4.1 */
		/* bit fields need to be truncated back to orignal value  d.t.*/
		if (q->in.left->tn.op == FLD)
		{	/* AND the result with a mask matching the field size */
			NODE	*z;

			z = talloc();
			*z = *p;
			p->in.left = z;		/* make AND node at top */
			p->tn.op = AND;
			p->in.right = talloc();	/* ICON node for mask */
			*p->in.right = *q->in.right;	/* copy ICON node */
			p->in.right->tn.lval = (1<<UPKFSZ(q->in.left->tn.rval)) -1;
		}
		if( odebug ) 
		{
			printf( "\nrewritten by rewass as:\n" );
	...
	*/
	...
	for( pi=inst,i=0; i<ninsav; ++i,++pi )
	{
		...
		{ /* non-asm template expansion */
			...
			if ( isvolatile = volatile_chk( p ) )
				protect(1);			/* issue no-optimize pragma */
			
			allo( p, q );
			expand( p, goal, q->cstring, q );

			if ( isvolatile )
				protect(0);			/* re-enable optimization */
		...
		if( c == CCC && p->tn.op != CCODES ) 
		{
 		...
 		else if( c>=0 && c<=NRGS && p->tn.op != REG ) 
/*		else if( c>=0 && c<=NRGS && !istnode( p ) )  old code */
		{
			cfix( p, NRGS );
		}
	}
