#ident	"@(#)reader.c	25.1	12/2/91 Copyright (c) 1991 by Arix Corp."
/*
    1.2	89/10/02 10:00:00 hossein
	Added support for M68040.

    1.1	89/06/08 00:38:33 root
	date and time created 89/06/08 00:38:33 by root

*/
/*	@(#)reader.c	7.4		*/	

static	char	SCCSID[] = "@(#)reader.c	7.4		85/08/26";
...
extern NODE node[TREESZ];
int fdef=0;
extern int ftnno;  /* number of current function */
extern int lineno;
...
int fast;
#ifdef IN_LINE
extern int asmdebug;  /* set if env var ASMDEBUG = YES */
extern FILE *inlfp;   /* file to store asm 'function' bodies */
#endif
int emuflag = 0;  /*flag for floating point emulation */
#ifdef m68k
#ifndef	M68040
# ifdef TWOPASS
#  ifdef DFLTDBLALIGN
int align_val = 32;	/* M68020 default variable alignment; word */
#  else
...
static union ndu * oreff _PARMS((union ndu *));
static union ndu * andeff _PARMS((union ndu *));
...
#ifdef FORCE881
	++is_m68881;    /* always generate code for the '881 */
#endif
#endif
...
#endif
				case 'F':
					++emuflag; /* emulation flag for fintrz and fscale instructions */
					break;
				default:
...
int negrel[] = 
{
	 NE, EQ, GT, GE, LT, LE, PL, MI, UGT, UGE, ULT, ULE
} ;  /* negatives of relationals */

/* bk: originally, there was a 4th, unused, argument, "register g"
 */
NODE *
genbr( o, l, p )
register NODE *p; 
...
register goal,t,f;
{
	/* gen code for conditionals in terms of GENLAB and GENBR nodes */
	...
	case GT:
		    ...
		    p->in.right->in.name == (char *) 0 ) 
		{
			...
			case LT:
			case GE:
				o = p->in.op = (o==GE)?PL:MI;
				goto dorel;
			case LE:
			case GT:
			  {	
				/* special hack: M68000 family BTST 
					sets only Z CCR bit */

				int con;

					   /* bit AND operator */
				if (       p->in.left->tn.op == AND
					&& p->in.left->in.right->tn.op == ICON
					&& p->in.left->in.right->tn.name == 0
					&& (con=p->in.left->in.right->tn.lval)
					&& (con & (con-1)) == 0
					/* with constant power of 2 */
				   )
						o = p->in.op = EQ;	
			    }
				/* fall into EQ/NE */
			...
			case EQ:
			case NE:
dorel:          if(    logop( p->in.left->tn.op )
	...
	if( files ){
		while( files < argc ) {
			...
			case 2:
				...
				break;
			case 3:
				if ((inlfp=fopen(argv[files],"r"))==NULL)
				{
					fprintf(stderr, "can't open asm macro temp file\n");
					exit(1);
				}
				break;
			default:
				cerror("usage error: c1 [<options>] <source file> <destination file> <macro tmp file : \"%s\" purpose unknown", argv[files]);
			}
			++files;
		}
	    }

	if (lflag || e2debug || odebug || rdebug || sdebug || udebug
	    || zflag || idebug)
			  setbuf(stdout,NULL);  /* no buffering if debugging */
	...
	case '#':
		{
			...
			if (strcmp( cstring, "pragma" ) == 0) {
				int keyidx;
				extern int noregsave;
				...
				keyidx = i;
				for ( ; isalpha(c) && i < sizeof(cstring)-1; c = getchar())
					cstring[i++] = c;
				cstring[i] = '\0';

				if ( strcmp( &cstring[keyidx], "noregsave" ) == 0 ) {
					noregsave = 1;
					while( (c=getchar()) > 0 )
						if( c == '\n' ) break;
					continue;
				} else if ( strcmp( &cstring[keyidx], "regsave" ) == 0 ) {
					noregsave = 0;
					while( (c=getchar()) > 0 )
						if( c == '\n' ) break;
					continue;
...
# endif
# ifndef NODBG
		if (idebug) e22print(p,"T");
		 else if ( p && odebug>2 ) e2print(p);
# endif
...
# endif
	...
	if( p ) 
	{
		/* expression does something */
		/* generate the code */
		p = chkscc(p);		/* strip CONV leading GENSCC */
# ifndef NODBG
		if( odebug>2 ) e2print(p);
# endif
		codgen( p );
...
# endif
	...
	switch( p->in.op)
	{
	case VAUTO:					/* read type qualifier info */
	case VPARAM:
		p->in.typeq = rdin(8);
		if( getchar() != '\n' ) cerror( "illegal \n" );
		break;
...
char *s;
{
	...
	else if (p->tn.goal == 0) printf(" NOGOAL\n");
