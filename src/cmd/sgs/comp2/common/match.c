#ident	"@(#)match.c	25.1	12/2/91 Copyright (c) 1991 by Arix Corp."
/*	@(#)match.c	7.3		*/
static	char	SCCSID[] = "@(#) match.c:	@(#)match.c	7.3	";
...
/* 	 MOTOROLA 4.1
 *   The following if is to pass up templates which indicate that they
 *   set condition codes, but in reality won't since an address register
 *   is the destination ( for address registers containing INTs ). The 
 *   tree that fits this situation looks like:
 *
 *                         op
 *		        /      \
 *                   REG int     ( whatever )
 *		    /   \
 *            USE_AREG   (register number) 
 *
 *    The op will be one of the following ( =, +=, -=, +, -, ++, -- )
 */
		if( (  p->in.op == ASSIGN
		    || p->in.op == ASG PLUS
		    || p->in.op == ASG MINUS
		    ...
		    && ( p->in.left->tn.op == REG )
		    && ( p->in.left->tn.lval & USE_AREG )
/*
 *   template indicates ( by a {$C} in stin ) that CC will be set
 */
		    && ( q->rewrite & RESCC )
/*
 *    There are two templates that do set CC when address registers are
 *    the destination: 1) cmp <ea>,%an 
 *    		and    2) tst %an
 *    The templates for these have the user need $U8 ( mfile2.h has
 *    the #defines for user needs ie #define CC_AREG 8 ) and will
 *    pass through this condition.
 */
		    && !((q->needs >> NEEDNUM) & CC_AREG )
		  )
		   continue;
/* 
 *   The following if is to pass up templates that would produce an add
 *   or subtract to memory by an address register ( for address registers
 *   containing INTs ). This case is identified by having a REG node with
 *   USE_AREG set in its lval on the right of the current op. The user need
 *   CC_AREG identifies the appropriate template that is needed for this
 *   special case. The tree that fits this situation looks like:
 *
 *                         op  ( +=, -=, ++, -- )
 *		        /      \
 *            ( memory )       REG int 
 ...
 *
 */
		else if( (  p->in.op == ASG PLUS
		         || p->in.op == ASG MINUS
		    	 || p->in.op == INCR
		    	 || p->in.op == DECR
		    	 )
		    	&& ( p->in.right )
		    	&& ( p->in.right->tn.op == REG )
		    	&& ( p->in.right->tn.lval & USE_AREG )
		    	&& !( p->in.left && ( p->in.left->tn.op == REG ))
			&& !((q->needs >> NEEDNUM) & CC_AREG )
		       )	
		   continue;
		else if( ( q->needs & USERNEEDS ) &&
		    ( ! user_need( p, q ) ) )
...
# ifndef STACK
		case 'D':	/* test for dependency */
			...
			/* bk: call to ushare was missing third arg: is this right now? */
			if( ushare( q2, q1->tn.rval, p->in.type ) == c ) continue;
			goto cream;
# endif
		...
		case 'R':  /* put an operand into a particular register */
			...
			if( c
			    ...
			    || ((q1->tn.lval&USE_AREG) ^ (q2->tn.lval&USE_AREG))
			  )
#else
			if( c
			    || ((q1->tn.type&TPOINT) ^ (q2->tn.type&TPOINT))
...
/*	 MOTOROLA 4.1
 *   templates with user need CC_AREG are to be ignored
 */ 
	if ( need & CC_AREG )
	   return( 0 );
/* roger: note that the loop checks all user needs. If one of
 * them fails, we can safely return 0, otherwise we keep checking
 * and finally return a 1
 */
	for( i = 1; need != 0; i <<= 1 )
	{
	  if (i & need)			/* if this need turned on ... */
		switch( i )
		{
#ifdef M68881
		case FP881:
		...
		case EMU_FP:
			if (!emuflag) return( 0 );
			else break; /* need satisfied */
		default:
			cerror( "unknown user need, %x\n", i );
		}
	  need &= ~i;				/* strip out handled need */
	}
