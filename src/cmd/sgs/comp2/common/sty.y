...
FILE *tbl_a,
     *tbl_a2,   /* added 10/1/85 by E.J.R. (BSD compiler ran out of space) */
     *tbl_b;
...
finished()
{
	...
	if( (tbl_a2 = fopen( "tablea2.c", "w" ) ) == NULL )
	{       fprintf( stderr, "sty: unable to open tablea2.c\n" );
		exit( -1 );
	}
	...
	fprintf( tbl_a2,"# include \"mfile2.h\"\n\n" );
	...
	fprintf( tbl_a2,"\n# define SHNL ((SHAPE *)0)\n" );
	fprintf( tbl_a2,"# define S(x) (&shapes[x])\n" );
	...
	fprintf( tbl_a2, "# define P(x) (&pshape[x])\n" );
	...
	for( j = 0, i = NSTYSHPS-1;  i > nmshp; --i ) {
		...
		fprintf( tbl_a2, "/*%4d %.8s */\t", j, shp[i].shname  );
		j = manylist( i, j );
		fprintf( tbl_a2, "SHNL,\n");
		++j;
	}
...
int	*cp;
{
	...
	if( shp[i].sop == o ) {
		fprintf( tbl_a2, "S(%d), ", i );
		if( !((++*cp)&07) ) fprintf( tbl_a2, "\n\t\t" );
		return( j+1 );
		}
	return( j );
	}
