...
/*							*/
/*	@(#) expr.c: version 25.1 created on 12/2/91 at 17:36:37	*/
/*							*/
/*	Copyright (c) 1990 by Arix Corporation		*/
/*	All Rights Reserved				*/
/*							*/
#ident	"@(#)expr.c	25.1	12/2/91 Copyright (c) 1990 by Arix Corporation"
/*							*/
...
#ident	"@(#)f77/f77/mach:expr.c	25.1"
...
while(--leng >= 0)
	if(*s != ' ')
		p->cons.ci = (p->cons.ci << shift) | hextoi(*s++);
return( (expptr) p );
}
...
if( ISCONST(realp) && ISNUMERIC(rtype) && ISCONST(imagp) && ISNUMERIC(itype) )
	...
	if( ISINT(rtype) )
		p->cons.cd[0] = realp->constblock.cons.ci;
	else	p->cons.cd[0] = realp->constblock.cons.cd[0];
	if( ISINT(itype) )
		p->cons.cd[1] = imagp->constblock.cons.ci;
	else	p->cons.cd[1] = imagp->constblock.cons.cd[0];
	}
else
	{
...
if(t == pt)
	...
	{
	q = (expptr) mkconst(t);
	consconv(t, &(q->constblock.cons),
		p->constblock.vtype, &(p->constblock.cons) );
	frexpr(p);
	}
#if TARGET == PDP11
...
switch(tag)
	...
	case TCONST:
		if(e->constblock.vtype == TYCHAR)
			{
			e->constblock.cons.ccp =
				copyn(1+strlen(e->constblock.cons.ccp),
					e->constblock.cons.ccp);
			e->constblock.vleng =
...
switch(p->tag)
	...
	case TCONST:
		if( ISCHAR(p) )
			{
			free( (charptr) (p->constblock.cons.ccp) );
...
#if TARGET==M68K
if(q->primblock.namep->vstg != STGCOMMON)
{
/* fprintf(stderr,"WIPING OUT memoffset\n"); */
/* fprintf(stdout,"#WIPING OUT memoffset\n"); */

			p->datap->addrblock.memoffset = (expptr) NULL;
}
else{
/* fprintf(stderr,"NOT WIPING OUT memoffset\n"); */
/* fprintf(stdout,"#NOT WIPING OUT memoffset\n"); */
}
#endif
		}
	else if(qtag==TPRIM && q->primblock.argsp==0 &&
...
if( ISICON(p) )
	{
	if(p->constblock.cons.ci < 0)
		goto badsub;
	if( ISICON(dimp->nelt) )
		if(p->constblock.cons.ci < dimp->nelt->constblock.cons.ci)
			return(p);
		else
			goto badsub;
	}
...
switch( p->vstg)
	...
	case STGAUTO:
		...
		if(p->vleng)
			...
			if( ISICON(t->vleng) )
				t->varleng = t->vleng->constblock.cons.ci;
			}
		return(t);
	...
	case STGAUTO:
		...
		if(t = v->vdim)
			if( (neltp = t->nelt) && ISCONST(neltp) )
				nelt = neltp->constblock.cons.ci;
			else
				dclerr("adjustable automatic array", v);
		p = autovar(nelt, v->vtype, v->vleng);
		v->voffset = p->memoffset->constblock.cons.ci;
		frexpr(p);
		break;
...

#define ICONEQ(z, c)  (ISICON(z) && z->constblock.cons.ci==c)
...
if(rp == 0)
	switch(opcode)
		{
		case OPNOT:
			lp->constblock.cons.ci = ! lp->constblock.cons.ci;
			return(lp);
...
switch(opcode)
	...
	case OPAND:
		p->cons.ci = lp->constblock.cons.ci &&
	...
	case OPCONCAT:
		ll = lp->constblock.vleng->constblock.cons.ci;
		lr = rp->constblock.vleng->constblock.cons.ci;
		p->cons.ccp = q = (char *) ckalloc(ll+lr);
		p->vleng = ICON(ll+lr);
		s = lp->constblock.cons.ccp;
		for(i = 0 ; i < ll ; ++i)
			*q++ = *s++;
		s = rp->constblock.cons.ccp;
		for(i = 0; i < lr; ++i)
			*q++ = *s++;
		break;
	...
	case OPPOWER:
		if( ! ISINT(rtype) )
			return(e);
		conspower(&(p->cons), lp, rp->constblock.cons.ci);
		break;
	...
	default:
		if(ltype == TYCHAR)
			{
			lcon.ci = cmpstr(lp->constblock.cons.ccp,
					rp->constblock.cons.ccp,
					lp->constblock.vleng->constblock.cons.ci,
					rp->constblock.vleng->constblock.cons.ci);
			rcon.ci = 0;
			mtype = tyint;
			}
		else	{
			mtype = maxtype(ltype, rtype);
			consconv(mtype, &lcon, ltype, &(lp->constblock.cons) );
			consconv(mtype, &rcon, rtype, &(rp->constblock.cons) );
			}
		consbinop(opcode, mtype, &(p->cons), &lcon, &rcon);
		break;
	}
...
switch(p->vtype)
	...
	case TYCOMPLEX:
	case TYDCOMPLEX:
		p->cons.cd[1] = - p->cons.cd[1];
		/* fall through and do the real parts */
	case TYREAL:
	case TYDREAL:
		p->cons.cd[0] = - p->cons.cd[0];
		break;
	default:
...
if(n < 0)
	...
	n = - n;
	consbinop(OPSLASH, type, &x, powp, &(ap->cons));
	}
else
	consbinop(OPSTAR, type, &x, powp, &(ap->cons));
...
switch(p->headblock.vtype)
	...
	case TYSHORT:
	case TYLONG:
		if(p->constblock.cons.ci > 0) return(1);
		if(p->constblock.cons.ci < 0) return(-1);
		return(0);

	case TYREAL:
	case TYDREAL:
		if(p->constblock.cons.cd[0] > 0) return(1);
		if(p->constblock.cons.cd[0] < 0) return(-1);
		return(0);

	case TYCOMPLEX:
	case TYDCOMPLEX:
		return(p->constblock.cons.cd[0]!=0 || p->constblock.cons.cd[1]!=0);
...
if(ISICON(rp))
	...
	if(rp->constblock.cons.ci < 0)
		...
		rp->constblock.cons.ci = - rp->constblock.cons.ci;
