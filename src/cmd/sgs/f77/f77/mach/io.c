...
/*							*/
/*	@(#) io.c: version 25.1 created on 12/2/91 at 17:38:07	*/
/*							*/
/*	Copyright (c) 1990 by Arix Corporation		*/
/*	All Rights Reserved				*/
/*							*/
#ident	"@(#)io.c	25.1	12/2/91 Copyright (c) 1990 by Arix Corporation"
/*							*/
...
if(p = V(IOSEND))
	if(ISICON(p))
		ioendlab = execlab(p->constblock.cons.ci) ->labelno;
	else
		err("bad end= clause");

if(p = V(IOSERR))
	if(ISICON(p))
		ioerrlab = execlab(p->constblock.cons.ci) ->labelno;
	else
		err("bad err= clause");
...
if(p = V(IOSFMT))
	...
	if(p->headblock.vtype == TYCHAR)
		...
		{
		if( (k = fmtstmt( mklabel(p->constblock.cons.ci) )) > 0 )
			{
...
if(statstruct)
	...
	if(p)
		praddr(initfile, p->vstg, p->memno,
			p->memoffset->constblock.cons.ci);
	else
