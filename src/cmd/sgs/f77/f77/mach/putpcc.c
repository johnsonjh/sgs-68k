...
/*							*/
/*	@(#) putpcc.c: version 25.1 created on 12/2/91 at 17:39:18	*/
/*							*/
/*	Copyright (c) 1990 by Arix Corporation		*/
/*	All Rights Reserved				*/
/*							*/
#ident	"@(#)putpcc.c	25.1	12/2/91 Copyright (c) 1990 by Arix Corporation"
/*							*/
...
#ident	"@(#)f77/f77/mach:putpcc.c	25.1"
...
switch(p->tag)
	...
	case TCONST:
		switch(type = p->constblock.vtype)
			...
			case TYLONG:
			case TYSHORT:
				p2icon(p->constblock.cons.ci, types2[type]);
				free( (charptr) p );
				break;
...
if(p->tag==TEXPR && p->exprblock.opcode==OPSTAR)
	...
	if(ISICON(rp) && lp->tag==TEXPR &&
	   lp->exprblock.opcode==OPPLUS && ISICON(lp->exprblock.rightp))
		...
		lp->exprblock.rightp->constblock.cons.ci = rp->constblock.cons.ci;
