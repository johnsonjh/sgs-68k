...
/*							*/
/*	@(#) put.c: version 25.1 created on 12/2/91 at 17:39:07	*/
/*							*/
/*	Copyright (c) 1990 by Arix Corporation		*/
/*	All Rights Reserved				*/
/*							*/
#ident	"@(#)put.c	25.1	12/2/91 Copyright (c) 1990 by Arix Corporation"
/*							*/
...
if(p->tag==TEXPR && p->exprblock.opcode==OPCONCAT)
	...
	return(p->headblock.vleng->constblock.cons.ci);
...
switch(type = p->vtype)
	{
	case TYCHAR:
		if(p->vleng->constblock.cons.ci > XL)
	...
	loop:
		...
		for(litp = litpool ; litp<lastlit ; ++litp)
			if(type == litp->littype) switch(litflavor)
				{
			case 1:
				if(p->vleng->constblock.cons.ci != litp->litval.litcval.litclen)
					break;
				if(! eqn( (int) p->vleng->constblock.cons.ci, p->cons.ccp,
			...
			case 2:
				if(p->cons.cd[0] == litp->litval.litdval)
					goto ret;
				break;

			case 3:
				if(p->cons.ci == litp->litval.litival)
					goto ret;
				break;
				}
		if(nliterals < MAXLITERALS)
			...
			switch(litflavor)
				...
				case 2:
					litp->litval.litdval = p->cons.cd[0];
					break;

				case 3:
					litp->litval.litival = p->cons.ci;
					break;
				}
			}
	default:
...
switch(type)
	...
	flpt:
		for(i = 0 ; i < k ; ++i)
			prconr(asmfile, type, p->cons.cd[i]);
		break;

	case TYCHAR:
		putstr(asmfile, p->cons.ccp,
			(int) (p->vleng->constblock.cons.ci) );
		break;

	case TYADDR:
		prcona(asmfile, p->cons.ci);
		break;
