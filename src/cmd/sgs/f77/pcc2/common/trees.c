...
/*							*/
/*	@(#) trees.c: version 25.1 created on 12/2/91 at 17:43:36	*/
/*							*/
/*	Copyright (c) 1990 by Arix Corporation		*/
/*	All Rights Reserved				*/
/*							*/
#ident	"@(#)trees.c	25.1	12/2/91 Copyright (c) 1990 by Arix Corporation"
/*							*/
/*	END NEW ARIX SCCS HEADER			*/
/*							*/

#ident	"@(#)sgs/comp2/common:trees.c	1.1"

/*	OLD MOT: trees.c	7.2		*/
/*static	char	SCCSID[] = "OLD MOT:  	OLD MOT: trees.c	7.2	";*/
...
# define OTHER 04000
# define NCVTR 010000
# define WHAT  080000		/* what is this?? ken#2: was hard-coded */
		...
		case NAME:
			...
			else
			{
				...
				case LABEL:
					...
					{
						char *ptr ,*malloc();
						ptr = malloc(25+strlen(sp->sname));
						sprintf(ptr,"L%%%s%%%d",sp->sname,ftnno);
						p->tn.name = ptr;
/* ken1: why were these two lines here???
						p->tn.lval = 0;
						p->tn.rval = -sp->offset;
*/
						p->tn.lval = -sp->offset;
...
register NODE *p;
{
	...
	{
		if (l->tn.op == FCON)
			l->tn.lval = l->fpn.dval != 0.0;
		else if (l->tn.op == ICON)
			l->tn.lval = l->tn.lval != 0;
		else
			return p;
		if ((o == ANDAND && l->tn.lval) || (o == OROR && !l->tn.lval))
		{
				/* transform into r != 0 */
			*l = *r;	/* copy fields */
	...
	/* float/double cases */
	else if ( l->tn.op == FCON && r->tn.op == FCON ) {
	    ...
	    switch( o ) {
	    case PLUS:	l->fpn.dval = lval + rval; break;
	    case MINUS:	l->fpn.dval = lval - rval; break;
	    case MUL:	l->fpn.dval = lval * rval; break;
	    case DIV:
		    if( rval == 0.0 )
			werror( "division by 0" );
		    else
			l->fpn.dval = lval / rval;
		    break;

	    case UNARY MINUS:
			l->fpn.dval = - lval; break;
	    case NOT:	l->tn.lval = lval == 0; break;

	    case LT:	l->tn.lval = lval <  rval; break;
	    case LE:	l->tn.lval = lval <= rval; break;
	    case GT:	l->tn.lval = lval >  rval; break;
	    case GE:	l->tn.lval = lval >= rval; break;
	    case EQ:	l->tn.lval = lval == rval; break;
	    case NE:	l->tn.lval = lval != rval; break;
	    default:
		    return(p);
	    }
	    if (logop(o)) {
		...
		l->fpn.dval = (float) l->tn.lval; /* truncate to FLOAT result */
...
# ifdef MYPRTREE
	...
	printf( "%d\t", p->in.op );
if (kdebug) printf("(inop=%s)\t",opst[p->in.op]); /* ken#1 */

	if( ty == LTYPE ) {
if (kdebug) {		/* ken#1 */
    printf("(LTYPE:");
    if (p->tn.rval == WHAT ) printf("WHAT");
    else printf("lval=%ld", p->tn.lval );
    printf(")\t");
}			/* endken1 */
		if (p->tn.rval == WHAT )
			printf("0\t");
...
if (kdebug) {	/* ken#1 */
	...
	printf("NAME||ICON||FCON" );
    else printf("rval=%d", p->tn.rval );
    printf(")\t");
}		/* endken#1 */
		if( p->in.op == NAME || p->in.op == ICON || p->in.op == FCON)
			printf( "0\t" );
...
if (kdebug) { 	/* ken#1 */
    printf("(type=");
    if ((p->in.type & ARY) == ARY) printf("ARY:");
    else if ((p->in.type & PTR) == PTR) printf("PTR:");
    else if ((p->in.type & FTN) == PTR) printf("FTN:");
    printf("%s)\t",db_intype[037 & (p->in.type)]);
}		/* endken#1 */
	printf( "%o\t", p->in.type );
...
# endif
	...
	case VPARAM:
		if( p->tn.rval == WHAT){ 
			p->tn.rval = p->tn.lval;
			p->tn.lval = 0;
/*fprintf(stderr,"This is the place %s \n",p->in.name);*/
			break;
		}
