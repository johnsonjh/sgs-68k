...
/*							*/
/*	@(#) optim.c: version 25.1 created on 12/2/91 at 17:43:05	*/
/*							*/
/*	Copyright (c) 1990 by Arix Corporation		*/
/*	All Rights Reserved				*/
/*							*/
#ident	"@(#)optim.c	25.1	12/2/91 Copyright (c) 1990 by Arix Corporation"
/*							*/
/*	END NEW ARIX SCCS HEADER			*/
/*							*/

#ident	"@(#)sgs/comp2/common:optim.c	1.1"

/*	OLD MOT: optim.c	7.2		*/
/*static	char	SCCSID[] = "OLD MOT:  OLD MOT: optim.c	7.2	";*/
...
# endif
	...
	if( t == lt ) goto paint;
/* 11.MAR.85: hanna
 * add this line so that "int x=(int)"abc"; " gets converted.
 */
/*
	if(t==INT && ISPTR(lt) && o== ICON ){fprintf(stderr,"# B1\n");}
	if(ISPTR(lt) && o== ICON ){fprintf(stderr,"# B1.old\n");}
*/
	if(t==INT && ISPTR(lt) && o== ICON ){printf("# B1\n");goto icon;}
/**/
...
# endif
	...
	case UGT:
	case UGE:
		/* hanna: don't trade sides on compare if the 
 		* operand is an icon of the proper size for a moveq
 		* and the operation is a long (or word?)
 		* code sequence		space	~time[best---worst]
 		* now:	mov.l	&ICON,	%dn	2	0,    2,    3
 		* 	cmp.bwl	%dn,	EA	2+EA	0+EA, 2+EA, 3+EA 
 		*                               ====    ====  ====  ====
 		*				4+EA    0+EA, 4+EA, 6+EA
 		* old: cmp.l	&ICON,	EA	6+EA	0+EA, 2+EA, 3+EA
		*      cmp.bw	&ICON,	EA	4+EA	0+EA, 2+EA, 3+EA
		*	where EA is the time to fetch both ICON and EA.
 		*/
		{
		register val = 0;
	/**/	if(l->in.type == INT || l->in.type == UNSIGNED){/* long size */
		if(val = l->tn.lval){
			if( ISCON(l) && ( val >= -128 && val <= 127) ) {
				fputs("#E3a\n",stdout);
				break;
				}
		}
