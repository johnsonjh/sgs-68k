...
/*							*/
/*	@(#) scan.c: version 25.1 created on 12/2/91 at 17:43:21	*/
/*							*/
...
#ident	"@(#)sgs/comp2/common:scan.c	1.1"

/*	OLD MOT: scan.c	7.3		*/
/*static	char	SCCSID[] = "OLD MOT:  scan.c:	OLD MOT: scan.c	7.3	";*/
...
#	include <string.h>
#	define TRUE	1
	int stkckflg=0;	/* global - referenced again in local.c - bfcode() */
#endif
...
/* sw9 */
...
#ifdef IN_LINE
	/* put out the inline file name */
	/* It must be a commented line that reads
	 *
	 * @INLINE tempfilename
	 *
	...
	/* sw9 */
	if ( Iflag ){
	if ((inlfp = fopen(inlfn =argv[argc-1],"w+")) == NULL)
	/*
	if ((inlfp = fopen(inlfn =tempnam( "/usr/tmp" , "inl"),"w+")) == NULL)
	*/
	{
...
onechar:
		...
		case A_LET:
			...
			*/
			...
			sp = &stab[id];
			/* sw0: checking typePrev allows
			** typedef int I; { int I; }
			*/
			if( sp->sclass == TYPEDEF && !stwart && typePrev <= 0 )
			{
				stwart = instruct;
