#ident	"@(#)symbols.c	25.1	12/2/91 Copyright (c) 1991 by Arix Corp."
/*	OLD MOT:symbols.c	7.1		*/
	...
	BYTE usrname;
{
	...
	do {
		...
		/* or the string table.					*/
			...
			hash = (hash + probe) % NHASH;			/* bk/marck */
...
{
register short index;
register short minsect = 2;
extern short symcnt;
	symbol * symptr = symtab;
	for (index=0; index < symcnt; ++index,symptr++) {
				/* bk: adapted to new (txtsec/txt2sec/txt3sec ordering */ 
		if ( symptr->sectnum > 1 ) {
			if ( symptr->sectnum < 4 ) {		/* txt?sec -> txtsec */
				symptr->sectnum = 1;
			} else if ( symptr->sectnum < 6 ) {	/* dat?sec -> datsec */
				symptr->sectnum = 2;
			} else {							/* adjust everything else */
				symptr->sectnum -= 3;
			}
		}
		if ( readonly && (symptr->sectnum == 2 ) )
			symptr->sectnum = 1;
	}
	if(readonly)
	for (index=0,symptr=symtab; index < symcnt; ++index,symptr++) {
	/* restore the data section number to 2 so the linker isn't confused */
		if(EQUAL(symptr->_name.name,".data",8)){
			symptr->sectnum=2;
			symptr->styp=3;
			break;
		}
