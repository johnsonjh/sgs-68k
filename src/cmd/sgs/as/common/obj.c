#ident	"@(#)obj.c	25.1	12/2/91 Copyright (c) 1991 by Arix Corp."
/*
    1.1	89/06/08 00:39:50 root
	date and time created 89/06/08 00:39:50 by root
...
extern unsigned short
	localmagic;	/* obj's magic num-- maybe usr defined (-M) */
extern short
...
extern long
	hdrptr,
	symbent,
	Zflag,		/* contains the time stamp, if any for header */
	gsymbent;

short
	Aflag = NO;	/* By default we leave the A1000 bit out */
...
static int  push _PARMS((struct symentag *));
...
headers()
{
	...
	if(Zflag)
		filhead.f_timdat=Zflag;
	else	time(&(filhead.f_timdat));
	filhead.f_flags = 0;

	/* invented to mark binaries with test bytes in -A a1000 world */
	if ( !Aflag )
		filhead.f_flags |= F_S3000;
	...
	if (need_mau)
		filhead.f_flags |= F_BM32MAU;
#endif
	filhead.f_magic = localmagic;	/* fill the magic number  */
					/* -M option:see pass0.c  */
					/* allow usr to set magic */
	outblock(&filhead, FILHSZ, fdout);
	for (i = 1, p = &sectab[1]; i <= seccnt; i++, p++)
	{
/*		if ( i == 2 )			/ bk: skip txt2sec /
			continue;		*/
	...
	register symbol *ptr;
{
	...
	if (sct > EXTERN) {	/* global defined symbols */
		...
		if(getindx(strptr,C_EXT)<0) {			/* beg ken#1 */
/* dux#1 */		if ((ptr->styp & TYPE) == ABS) 
					sct = N_ABS;
	 		else
					sct= ptr->sectnum;	/* end dux#1 */
								/* end ken#1 */
