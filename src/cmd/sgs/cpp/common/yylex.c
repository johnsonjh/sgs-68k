#ident	"@(#)yylex.c	25.1	12/2/91 Copyright (c) 1991 by Arix Corp."
/*
    1.1	89/06/08 00:41:20 root
	date and time created 89/06/08 00:41:20 by root
...
yylex()
{
	...
	for ( ;; )
	{
		...
		*newp = '\0';
		...
		{
			if ( *inp == '0' )
				yylval.numb.val = ( inp[1] == 'x' || inp[1] == 'X' ) ?
					tobinary( inp + 2, 16 ) :
					tobinary( inp + 1, 8 );
			else
				yylval.numb.val = tobinary( inp, 10 );
			val = number;
		}
		else if ( isid( *inp ) )
		{
			...
			else
			{
				...
				ref(inp, xline);
#endif
				yylval.numb.val = ( sp->value == 0 ) ? 0 : 1;
				val = number;
			}
		}
		else if ( *inp == '\'' )	/* character constant */
		{
			...
			if ( inp[1] == '\\' )	/* escaped */
			{
				...
				if ( inp[2] <= '9' && inp[2] >= '0' )
					yylval.numb.val = c = tobinary( inp + 2, 8 );
				else
					yylval.numb.val = inp[2];
			}
			else
				yylval.numb.val = inp[1];
		}
		else if ( strcmp( "\\\n", inp ) == 0 )
		{
			*newp = savc;
		...
		else
		{
			*newp = savc;
			if ( *inp != TOKSEP )
				pperror( "Illegal character 0x%x in preprocessor if",
					*inp );
			continue;
		}
	ret:
		...
		*newp = savc;
		outp = inp = newp;
		yylval.numb.unsign = unsigned_flag;
		unsigned_flag = 0;
		return( val );
	}
}
	...
	char *st;
{
	...
	while ( c = *s++ )
	{
		...
		case '8': case '9':
			if (b <= 8 && !warned) {
				ppwarn( W_DFLT, "Illegal octal number %s", st);
				warned = 1;
			}
