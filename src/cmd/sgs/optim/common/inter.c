#ident	"@(#)inter.c	25.1	12/2/91 Copyright (c) 1991 by Arix Corp."
/*	inter.c	6.1		*/
...
{ /* add text ref to reference list */
	...
	void
filter() 
{ /* print FILTER nodes and remove from list */
...
{ /* print text list */
	...
	for (ALLN(p)) {
		switch (p->op) {
			case LINK_W:			/* jcl: if retaining link/unlk */
			case LINK_L:
				prinst(p);
				expanmovm(funclive, true);
				break;
			case UNLK:
				expanmovm(funclive, false);
				prinst(p);
				break;
			default:
				prinst(p);
...
same(p, q) 
NODE *p, *q; 
{ /* return true iff nodes are the same */
...
sameaddr(p, q) 
NODE *p, *q; 
{ /* return true iff ops[1...] are the same */
...
xfree(p) 
char *p; 
{ /* free up space allocated by xalloc */
