#ident	"@(#)nlist.c	25.1	12/2/91 Copyright (c) 1991 by Arix Corp."
...
/*	ATT:#ident	"libc-port:gen/nlist.c	1.16"	*/
...
#endif
/* hanna, just catch them all ! */
#ifndef ISMAGIC
#define ISMAGIC(x)	ISCOFF(x)
#endif
#if u3b || vax || u3b5 || m68 || m68k || M68020 || 1 /* hanna arete uses this */
...
#if vax || u3b || u3b5 || m68 || m68k || M68020 || 1
#	ifndef FLEXNAMES
#		define FLEXNAMES 1
#	endif
...
#if !(u3b || vax || u3b5 || m68 || m68k || M68020 || 1)
	...
	struct nlist *list;
{
	...
	(void) close(fd);
#if (vax || u3b || u3b5 || m68 || m68k || M68020 || 1) && FLEXNAMES
	if (strtab != (char *)0)
		free(strtab);
#endif
	return (0);
