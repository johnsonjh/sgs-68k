#ident	"@(#)ftw.c	25.1	12/2/91 Copyright (c) 1991 by Arix Corp."
/*
 ...
 */
#ident	"@(#)ftw.c	3.1 LAI System V NFS Release 3.2/V3 source"
#ident	"@(#)ftw.c	2.2 System V NFS source"
...
int depth;
{
	 ...
	 * It's ok to have a symbolic link that points to non-existing file.
	 * In this case, pass FTW_NS to a function instead of aborting ftw()
	 * right away. 
	 */
	if(stat(path, &sb) < 0) {
		save_errno = errno;
	 ...
	 */
	while((errno = 0, (dir = readdir(fd))) != NULL) {
		if(dir->d_ino != 0 && strcmp(dir->d_name, ".") != 0 &&
			...
			if(depth <= 1) {
				...
				if(errno = 0, seekdir(fd, here), errno != 0) {
					(void)closedir(fd);
					free(subpath);
					return(-1);
				}
	...
	/*
	 *	We got out of the subdirectory loop.  The errno from
	 *	the final read is in save_errno.  Clean up and then check if
	 *	the final read was successful.  If not, give an
	 *	error return.
	 */
	free(subpath);
	if(closedir(fd) != NULL || save_errno != 0)
		return(-1);
	return(0);
}
