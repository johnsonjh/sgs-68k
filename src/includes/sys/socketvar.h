...
#ifdef  NO_SCCS_ID
#ident "@(#)socketvar.h  (TWG)      1.2     89/06/14 "
#endif  /*NO_SCCS_ID*/
...
/*
 *  Fields used by TLI interface
 */
	short	so_event;		/* Most recent asynchronous event*/
	short so_usrstate;		/* socket state as seen by the user */
	int	so_seq;			/* sequence number of connections on */
					/* a accept queue 		*/
};
...
#define sosetevent(so,event)	so->so_event |= event
